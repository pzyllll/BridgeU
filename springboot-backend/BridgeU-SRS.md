...
**URS-33 (UC-24 – View Followers/Friends List)**: The User can view their followers list and mutual follows list, and manage follow relationships.
...
**SRS-121**: The system shall allow users to perform follow/unfollow actions from the followers/mutual follows list, updating the relationship status in real-time, and for mutual follows (Friends) shall open or create a private conversation when the "Send Message" action is clicked, delegating message permission enforcement to the Private Messaging Rules defined in Section 5.3.

**SRS-122**: If the user has no followers or mutual follows, the system shall display an empty state message: "No followers" (zh: "暂无关注者") or "No mutual follows" (zh: "暂无互相关注").

**SRS-123**: If the request to fetch followers/mutual follows fails (network/server error), the system shall display a localized error message and provide a retry action.
...
| Last Revision Date | |
| Actors | User |
| Description | User views their followers list and mutual follows list, and manages follow relationships. |
| Trigger | User clicks "View Followers" or "View Mutual Follows" on the profile page. |
...
| Input | type | Constraint | Example |
|-------|------|------------|---------|
| User ID | String | Optional, for viewing another user's followers | "user123" |
| Search Query | String | Optional, for filtering mutual follows | "john" |

**Post conditions**:
- User views a list of followers or mutual follows with complete user information.
- Follow/unfollow actions update relationship status in real-time.
- From the mutual follows (Friends) list, the user can start or continue private chats with Friends; messaging behavior follows the Private Messaging Rules.
...
| 3. User clicks "View Followers" or "View Mutual Follows". | 4. System opens user list modal or navigates to dedicated page and sends request to appropriate endpoint (`GET /api/users/{userId}/followers` or `GET /api/users/mutual-follows`).<br>   • `[E1: Network timeout or connection error]`<br>   • `[E2: Unauthorized]` |
| | 5. Backend retrieves followers or mutual follows list and returns user list.<br>   • `[E3: Server error]` |
| 6. User views the user list. | 7. System displays user cards with:<br>   - User ID, username, display name, avatar<br>   - Follow/unfollow action buttons (if applicable)<br>   - Search box (for mutual follows)<br>   • `[A1: No followers/mutual follows]` |
| 8. User can click "Send Message" on a mutual follow (Friend) card to start or continue a private chat (optional). | 9. System opens or creates a private conversation (UC-15) for the selected Friend and navigates to the messaging interface, enforcing the Private Messaging Rules (Section 5.3). |
| 10. User can search/filter mutual follows (optional). | 11. System sends search request with `q` parameter and updates displayed list. |
| 12. User clicks follow/unfollow on a user card. | 13. System sends follow/unfollow request and updates UI on success.<br>   • `[E1: Network timeout or connection error]` |
| 14. User continues viewing or closes the list. | 15. System maintains the list state and updates relationship indicators. |

**Alternative Flow**

**`[A1: No followers/mutual follows]`**
- A1.1 System detects that the user has no followers or mutual follows.
...
**`[E1: Network timeout or connection error]`**
- E1.1 Request to fetch followers/mutual follows or follow/unfollow action fails due to network issues.
- E1.2 System displays a localized error message and provides a retry action.
- E1.3 Use case end.
...
**Note**:
- The system provides separate views for "View Followers" (users who follow the current user) and "View Mutual Follows" (users who mutually follow the current user).
- Followers list is loaded via `GET /api/users/{userId}/followers`, while mutual follows list is loaded via `GET /api/users/mutual-follows` with optional search filtering via the `q` parameter.
- User cards display: user ID, username, display name, avatar, and follow/unfollow action buttons (if applicable based on the relationship status).
- Users can perform follow/unfollow actions directly from the followers/mutual follows list, and the relationship status updates in real-time across all views.
- For mutual follows (Friends), user cards also provide a "Send Message" action that opens the private messaging interface; actual messaging limits and permissions follow the Private Messaging Rules in Section 5.3.

##### 2.3.24.3 Activity Diagram

```plantuml
@startuml UC24_Activity_Diagram
title UC-24: View Followers/Friends List - Activity Diagram

|User|
start
:opens profile page;
:clicks "View Followers"\nor "View Mutual Follows";

|System|
:frontend opens user list modal\nor navigates to page;
:frontend sends request to\nGET /api/users/{userId}/followers\nor GET /api/users/mutual-follows;

if () then ([Request failed])
  :shows error message\nand allows retry;
  stop
else ([Request succeeded])
  :backend retrieves followers\nor mutual follows list;
  
  if () then ([No users])
    :shows empty state\n"No followers" or "No mutual follows";
    stop
  else ([Users available])
    :displays user cards\nwith user info and action buttons;
    
    |User|
    if () then ([Clicks "Send Message" on Friend])
      |System|
      :opens or creates private conversation\nfor the selected Friend;
      :navigates to messaging UI\nand enforces Private Messaging Rules;
      stop
    else ([Does not click "Send Message"])
      if () then ([Searches/filters])
        |System|
      ...
      |User|
      if () then ([Clicks follow/unfollow])
        |System|
        :sends follow/unfollow request;
        
        if () then ([Action failed])
          :shows error;
          stop
      ...
      else ([Does not click follow/unfollow])
        stop
      endif
    endif
  endif
